...
//   struct proc *p = myproc();
//   int pid;
//   int created = 0; // Track the number of successfully created processes

//   struct proc *children[n]; // Array to store pointers to created child processes

//   for (int i = 0; i < n; i++) {
//     // Allocate process
//     printf("Allocating process %d\n", i + 1);
//     struct proc *np;
//     printf("Allocating 2 process %d\n", i + 1);

//     if ((np = allocproc()) == 0) {
//       // Cleanup already allocated processes on failure
...
//     // Copy user memory from parent to child
//     if (uvmcopy(p->pagetable, np->pagetable, p->sz) < 0) {
//       printf("Failed to copy user memory\n");
//       freeproc(np);
//       release(&np->lock);
//       // Cleanup already allocated processes on failure
//       for (int j = 0; j < created; j++) {
...
//     // Cause fork to return the child number in the child process
...
//     // Store the child process pointer for later cleanup or release
//     children[created++] = np;

//     // Copy the PID to the user-space array
//     if (copyout(p->pagetable, (uint64)(pids + i * sizeof(int)), (char *)&pid, sizeof(int)) < 0) {
//       printf("Failed to copy PID to user space\n");
//       freeproc(np);
//       release(&np->lock);
//       // Cleanup already allocated processes on failure
//       for (int j = 0; j < created; j++) {
...
//   // Release all created child processes to the runnable state
...
//     printf("Child %d is now runnable\n", children[i]->pid);
...
int forkn(int n, int *pids)
{
  ...
  wakeup(Lock); // Wake up all processes sleeping on the parent
...
int waitall(uint64 childsAddress, uint64 childsStauts)
{
  ...
  while (1)
  {
    ...
    if (number_of_kids == 0 || number_of_kids == number_of_zombies)
      break;

    sleep(p, &wait_lock); // sleep
  }

  // collect the exit status of all children
  for (pp = proc; pp < &proc[NPROC]; pp++)
  {
    if (pp->parent == p && pp->state == ZOMBIE)
    {
      ...
      // copy the exit status to the user space
      if (childsStauts != 0 &&
      ...
      freeproc(pp); // clean up the child process
      release(&pp->lock);
    }
  }

  release(&wait_lock);  // release the wait lock
  return childsCounter; // return the number of children
}
